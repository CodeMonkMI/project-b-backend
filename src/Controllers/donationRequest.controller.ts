...
export class DonationRequestController {
  ...
  async all(req: Request<{}, {}, {}, AllReqQuery>, res: Response) {
    ...
    try {
      const data = await this.prisma.donationRequested.findMany({
        where: {
  ...
  async create(req: Request<{}, {}, CreateReqBody>, res: Response) {
    ...
    try {
      const item = await this.prisma.donationRequested.create({
        data: {
          address,
          blood,
          date,
  ...
  async single(req: Request<{ id: string }>, res: Response) {
    try {
      ...
      const single = await this.prisma.donationRequested.findUnique({
        where: {
          id,
        },
        select: {
  ...
  async approve(
    ...
    try {
      const item = await this.prisma.donationRequested.update({
        where: {
          id: req.params.id,
        },
        data: {
  ...
  async complete(
    ...
    try {
      const donationRequest = await this.prisma.donationRequested.findFirst({
        where: {
          id: req.params.id,
          status: "ready",
        ...
        data: [
          {
            ...
            createdById: user.id,
            receiverId:
              item.requestedBy?.Profile?.id == user.id ? [user.id] : [],
          },
          {
  ...
  async progress(req: Request<{ id: string }, {}, {}>, res: Response) {
    try {
      const id = req.params.id;
      const findRequest = await this.prisma.donationRequested.findFirst({
        where: {
          id,
        },
      });
  ...
  async assign(req: Request<{ id: string }, {}, AssignReqBody>, res: Response) {
    ...
    try {
      ...
      const donorData = await this.prisma.user.findUnique({
        where: {
          id: donor,
        },
        select: {
  ...
  async findDonor(req: Request<{}, {}, FindReqBody>, res: Response) {
    ...
    try {
      const donationRequests = await this.prisma.user.findMany({
        where: {
          isVerified: true,
          OR: [{ deleteAt: { isSet: false } }, { deleteAt: null }],
          Profile: {
  ...
  async userContribution(
    ...
    try {
      ...
      const donationCount = await this.prisma.donationRequested.aggregate({
        _count: true,
        where: {
          status: "completed",
          donor: {
      ...
      const donationCountThisMonth =
        await this.prisma.donationRequested.aggregate({
          _count: true,
          where: {
            status: "completed",
            donor: {
      ...
      const requestCount = await this.prisma.donationRequested.aggregate({
        _count: true,
        where: {
          requestedBy: {
            username,
